...
public class UpdateCommand extends Command{
    ...
    public void execute(String args){
        if (args == null) {
            userIO.println("Необходимо указать id группы");
            return;
        }
        Long id;
        try{
            ...
            userIO.println("Некорректный ввод. Необходимо указать id группы.");
            return;
        }
        // TODO CHECKING EXISTING GROUP
        if(!collectionManager.groupExists(id)){
            userIO.println("Группа с таким id не существует!");
            return;
        }
        StudyGroup toAdd;
        try {
        ...
        try{
            collectionManager.setGroup(id,toAdd);
        } catch (WrongInputException e){
            ErrorHandler.logError(e.getMessage());
            return;
        }
