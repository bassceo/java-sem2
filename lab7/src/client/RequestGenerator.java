    ...
    public Queue<Request> processCommand(String command) throws RecursiveException{
        ...
        if(commands.contains(commandName)){
            if(commandName.equals("add")){
                ...
                requests.add(new Request(commandName, args, createStudyGroup(args), login, password));
                ...
                requests.add(new Request(commandName, args, createStudyGroupForUpdate(args), login, password));
            }else if(commandName.equals("login") || commandName.equals("register")){
                authorization(commandName);
            }
            else {
                requests.add(new Request(commandName, args, login, password));
            }
        }else{
            throw new RecursiveException("Эта команда не существует: " + commandName);
        }
        return requests;
    }

    private void authorization(String type){
        login = readInputParameter("Введите логин", false, String::trim, value -> value != null);
        password = readInputParameter("Введите пароль", false, String::trim, value -> value != null);
        requests.add(new Request(type, login, password));
    }
